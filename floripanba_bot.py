...
def _slug_from_g(g: dict) -> str: return f"nba-{(g.get('awayTeam',{}).get('teamTricode','') or '').lower()}-{(g.get('homeTeam',{}).get('teamTricode','') or '').lower()}-{date.today().isoformat()}"

def polymarket_props_today_from_scoreboard() -> List[Prop]:
    today = date.today().isoformat(); now = now_ts()
    if PM_CACHE["date"] == today and (now - PM_CACHE["ts"]) < 8 * 60: return PM_CACHE["props"]
    ...
    for g in games:
        slug = _slug_from_g(g)
        try:
            r = SESSION_PM.get(f"{GAMMA}/events/slug/{slug}", timeout=10)
            if r.status_code == 200:
                ev = r.json()
                mkts = ev.get("markets", [])
                if not mkts and ev.get("id"):
                    rm = SESSION_PM.get(f"{GAMMA}/markets", params={"event_id": ev["id"], "limit": 200}, timeout=10)
                    mkts = rm.json() if rm.status_code == 200 else []
                for m in mkts:
                    ...
                    player = m.get("groupItemTitle") or ""
                    if not player:
                    ...
                    if player and line_val:
                        ...
                        props_all.extend([Prop(player, tipo, line_val, "over", "polymarket", slug, str(m.get("id"))),
                                          Prop(player, tipo, line_val, "under", "polymarket", slug, str(m.get("id")))])
        except: pass

    uniq = []
    seen = set()
    for p in props_all:
        if (p.game_slug, p.player.lower(), p.tipo, p.line, p.side) not in seen:
            seen.add((p.game_slug, p.player.lower(), p.tipo, p.line, p.side))
            uniq.append(p)

    if not uniq:
        log.warning("Polymarket falló. Usando Fallback.")
...
async def cmd_odds(u: Update, c: ContextTypes.DEFAULT_TYPE):
    msg = await u.message.reply_text("⏳ *Cargando Polymarket...*", parse_mode=ParseMode.MARKDOWN)
    props = await asyncio.to_thread(polymarket_props_today_from_scoreboard)
    if not props: return await msg.edit_text("❌ No hay props.")
...
async def show_game_props_advanced(u: Update, c: ContextTypes.DEFAULT_TYPE, slug: str, props: List[Prop]):
    ...
    try: results = await asyncio.gather(*[_sf(pl, lns) for pl, lns in ul.items()], return_exceptions=True)
    except Exception as e: return await msg.edit_text(f"❌ Error de timeout: {e}")

    pd = {}
    for r in results:
        if not isinstance(r, Exception) and r[1]: pd[r[0]] = r[1]
    if not pd: return await msg.edit_text("❌ La API de la NBA bloqueó la IP temporalmente. Intenta en unos minutos.")
...
async def cmd_live(u: Update, c: ContextTypes.DEFAULT_TYPE):
    ...
    except: return await msg.edit_text("⚠️ Error scoreboard")
    
    live_games = [g for g in games if g.get("gameStatus") == 2]
    if not live_games: return await msg.edit_text("⏸️ No hay partidos en vivo ahora.")
    
    await msg.edit_text(f"🔄 *{len(live_games)} partido(s) en vivo* — leyendo boxscores...", parse_mode=ParseMode.MARKDOWN)
    props = await asyncio.to_thread(polymarket_props_today_from_scoreboard)
    
    pbn = {}
    for p in props: pbn.setdefault(p.player.lower(), []).append(p)
        ...
        for tk in ["homeTeam", "awayTeam"]:
            for pl in box.get(tk, {}).get("players", []):
                ...
                for pr in m:
                    ...
                    pv, meta = await asyncio.to_thread(pre_score, pl.get("personId"), pr.tipo, pr.line, pr.side)
                    if pr.side == "over":
                        delta = pr.line - act
                        if 0.5 <= delta <= 4.0:
                            f = int(clamp(0.55 * 80 + 0.45 * pv, 0, 100)) # Simplified live calc for speed
                            sr.append((f, pr, act, delta, q, clk, diff, meta))
                    else:
                        delta = pr.line - act
                        if delta >= 2.0:
                            f = int(clamp(0.65 * 80 + 0.35 * pv, 0, 100))
                            sr.append((f, pr, act, delta, q, clk, diff, meta))

    await msg.delete()
    if not sr: return await u.message.reply_text("📭 *Sin señal en vivo ahora*")
    sr.sort(key=lambda x: x[0], reverse=True)
    
    ls = [f"🔥 *LIVE — {len(live_games)} partido(s)*\n{'─'*28}"]
    for (f, pr, act, d, q, clk, df, m) in sr[:15]:
        lbl = "faltan" if pr.side == "over" else "colchón"
        ls.append(f"\n{_e(f)} `{f}/100` — *{pr.player}*\n"
                  f"{'🏀' if pr.tipo=='puntos' else '💪'} {pr.tipo.upper()} {pr.side.upper()} `{pr.line}` | actual `{act:.0f}` ({lbl} `{d:.1f}`)\n"
                  f"⏱️ Q{q} {clk} | Dif `{df}` | H5: `{m.get('hits5','?')}`")
    await _send_long(u, "\n".join(ls))
...
async def cmd_bet(u: Update, c: ContextTypes.DEFAULT_TYPE):
    b = re.sub(r"^/bet(@\w+)?\s*", "", u.message.text or "").strip()
    p = [x.strip() for x in b.split("|")]
    if len(p) < 4: return await u.message.reply_text("Uso: `/bet Jugador | tipo | side | linea | monto`", parse_mode=ParseMode.MARKDOWN)
    pl, tp, sd, ln = p[0], p[1].lower(), p[2].lower(), float(p[3])
    am = float(p[4]) if len(p)>=5 else 1.0
    
    msg = await u.message.reply_text("⏳ Registrando...", parse_mode=ParseMode.MARKDOWN)
    pid = await asyncio.to_thread(get_pid_for_name, pl)
    if not pid: return await msg.edit_text("⚠️ Jugador no encontrado.")
    
    pv, _ = await asyncio.to_thread(pre_score, pid, tp, ln, sd)
    
    bet = Bet(_new_bet_id(), u.effective_user.id, pl, tp, sd, ln, am, pv, "", now_ts())
    bets = load_bets(); bets.append(bet); save_bets(bets)
    await msg.edit_text(f"✅ *Apuesta Registrada #{bet.id}*\n👤 {pl} | {tp.upper()} {sd.upper()} `{ln}` | 💰 `{am}u`\n{_e(pv)} PRE: `{pv}/100`", parse_mode=ParseMode.MARKDOWN)
...
async def background_autoresolve_bets(context: ContextTypes.DEFAULT_TYPE):
    ...
    bets = load_bets()
    pnd = [b for b in bets if not b.result]
    if not pnd: return
    try:
        games = await asyncio.to_thread(lambda: scoreboard.ScoreBoard().get_dict()["scoreboard"]["games"])
        fin = {f"nba-{g.get('awayTeam',{}).get('teamTricode','').lower()}-{g.get('homeTeam',{}).get('teamTricode','').lower()}-{date.today().isoformat()}": g.get("gameId") for g in games if g.get("gameStatus") == 3}
    except: return

    resolved = False
    for b in pnd:
        # Simplificación de autoresolve (asumiendo matching de boxscore manual)
        pass # Por seguridad y limpieza de threads en background, requiere boxscore lookup.

async def background_smart_alerts(context: ContextTypes.DEFAULT_TYPE):
    cid = context.job.chat_id
    # Se ejecuta en background buscando PRE > 68 (requiere fetch general)
    pass
